"""Native cognis-connect emit for filecarve — forward findings to any platform.

Maps filecarve's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    filecarve ... --format json | filecarve-emit --to stix
    filecarve-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        out = dict(rec)
        out['title'] = rec.get('name', '') + (' (truncated)' if rec.get('truncated', False) else '')
