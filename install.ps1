# Comprehensive installer for cognis-digital/filecarve (Windows PowerShell).
# Tries: pipx -> uv -> pip (git+https) -> from source.
# filecarve is source-available and not on PyPI; all paths install from GitHub.
$ErrorActionPreference = "Stop"
$Repo = "filecarve"
$Url  = "git+https://github.com/cognis-digital/filecarve.git"
$Git  = "https://github.com/cognis-digital/filecarve.git"
...
if (Have pipx) {
  ...
  pipx install $Url; if ($LASTEXITCODE -eq 0) { Say "Done. Run: filecarve"; exit 0 }
}
if (Have uv) {
  Say "Installing with uv..."
  uv tool install $Url; if ($LASTEXITCODE -eq 0) { Say "Done. Run: filecarve"; exit 0 }
}
if (Have pip) {
  Say "Installing with pip (user site)..."
  pip install --user $Url; if ($LASTEXITCODE -eq 0) { Say "Done. Run: filecarve"; exit 0 }
}
